...
async function runImport() {
  ...
  for (const batch of result.batches) {
    for (const item of batch.items) {
      ...
      // Create purchase if we have a price
      if (item.price) {
  ...
  console.log('\n=== IMPORT COMPLETE ===');
  console.log(`New wines: ${wineCount}`);
  console.log(`New vintages: ${vintageCount}`);
  console.log(`Purchase records: ${purchaseCount}`);
  console.log(`Tasting records: ${tastingCount}`);

  // Get final counts
  const totalWines = await prisma.wine.count();
  const totalVintages = await prisma.vintage.count();
  const totalPurchases = await prisma.purchaseItem.count();
  const totalTastings = await prisma.tastingEvent.count();
  const withPrices = await prisma.vintage.count({
    where: { purchaseItems: { some: {} } }
  });

  console.log('\n=== DATABASE TOTALS ===');
  console.log(`Total wines: ${totalWines}`);
  console.log(`Total vintages: ${totalVintages}`);
  console.log(`Total purchases: ${totalPurchases}`);
  console.log(`Total tastings: ${totalTastings}`);
  console.log(`Vintages with prices: ${withPrices}`);
